...
class Ftp extends EventEmitter {
  ...
  async put(from, destination) {
    ...
    if (from instanceof ArrayBuffer) {
      // console.log("is arraybuffer, creating socket with data ", Buffer.from(from))
      return this.getPutSocket(Buffer.from(from), destination);
    } else if (from instanceof Buffer) {
      return this.getPutSocket(from, destination);
    } else if (from instanceof Uint8Array) {
      // console.log("is uint8array, passing buffer back")
      return this.put(from.slice(0).buffer, destination)
    } else if (from instanceof Blob) {
      return this.put(await from.arrayBuffer(), destination);
  ...
  async getPutSocket(from, path) {
    ...
    //         socket.write(from.slice(0).buffer);
    //         socket.close();
    //       } else if (from instanceof Readable) {
    //         from.pipe(socket);
    //       }
    ...
    // });
    const socket = await this.getPasvSocket();
    this.execute(`stor ${path}`)

    // You've heard of the hackyCloseHandler, now introducing, hackyOpenHandlers
    const res = await new Promise((resolve, reject) => {
      const dec = new TextDecoder();
      const hackyOpenHandler = (data) => {
        if (dec.decode(data).startsWith("150") || dec.decode(data).startsWith("125")) {
          resolve({code: 150});
          this.socket.off("data", hackyOpenHandler);
        }
      }
      this.socket.on("data", hackyOpenHandler);
    })
    ...
    if (res.code === 125 || res.code === 150) {
      ...
      throw (new Error("Unexpected command " + res.text));
    }
